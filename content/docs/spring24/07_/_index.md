...
In LoRA, {{< katex >}} $W_o$ {{< /katex >}} matrix usually corresponds to {{< katex >}} $W_Q$, $W_K$, $W_V$ {{< /katex >}}, or  {{< katex >}} $W_O$ {{< /katex >}}, query, key, value, and output projection matrices of attention as opposed to Feed Forward Networks (FFN) matrices as hidden size of FFNs tend to be much larger then projection matrices of attentions. The most common practice seems to be injecting LoRA into query and value though exact implementation can vary widely.
...
{{< katex>}} $\text{LoREFT}(h) = h + R^{T} (W h + b - R h)$ {{< /katex >}}, where {{< katex >}} $R \in \mathbb{R}^{r \times d} and W \in \mathbb{R}^{r \times d}${{< /katex >}}$ {{< /katex >}} are low rank matrices. 
